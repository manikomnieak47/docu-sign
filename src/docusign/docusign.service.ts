...
export class DocuSignService  {
  ...
  //       null,
  //     );
  //     return Buffer.from(docData, 'binary');
  //   } catch (exception) {
  //     console.log({ line: 'DocuSign error', data: exception?.response || exception });
  //     throw exception;
  //   }
  // }
  ...
  async requestSignature(req): Promise<string> {
    try {
      ...
      console.log("this.config.accountId = ", this.config.accountId, envelopeOptions)
      const result = req.identity
        ? await envelopeApi.update(this.config.accountId, req.identity, { ...envelopeOptions, resendEnvelope: 'true' })
        : await envelopeApi.createEnvelope(this.config.accountId, envelopeOptions);
...
//       envelopesApi.update(this.config.accountId, id, env);
//     } catch (exception) {
//       console.log({ line: 'DocuSign error', data: exception?.response || exception });
//       throw exception;
//     }
//   }
  ...
  private async makeEnvelope(email: string, name: string, documentBuffer: Buffer, metadata: any) {
